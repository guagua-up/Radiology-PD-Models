...
# 读取数据
data <- read.csv("D:/PD/Imagings/SUVR/CentrumSemiovale_1/HC_PIGD/HC_PIGD_Wholebrain/MODELS/FinalFeature.csv") %>% 
  select(-(1:4)) %>% 
  mutate(Diagnosis = factor(.[,1], levels = c(0, 1), labels = c("HC", "PIGD"))) %>% 
  rename(Diagnosis = 1)
...
calculate_metrics <- function(model) {
  ...
  fold_stats <- model$pred %>%
    ...
    summarise(
      ...
      Sensitivity = sum(obs == "PIGD" & pred == "PIGD", na.rm = TRUE) / 
        sum(obs == "PIGD", na.rm = TRUE),
      Specificity = sum(obs == "HC" & pred == "HC", na.rm = TRUE) / 
        sum(obs == "HC", na.rm = TRUE),
      AUC = as.numeric(pROC::auc(pROC::roc(obs, PIGD, quiet = TRUE))),
      .groups = "drop"
    )
...
permutation_test <- function(model, data, n_perm = 1000) {
  obs_prob <- predict(model, data, type = "prob")[, "PIGD"]
  ...
  perm_aucs <- foreach(i = 1:n_perm, .combine = 'c',
                       .packages = c("pROC", "caret")) %dopar% {
                         ...
                         perm_prob <- predict(model, perm_data, type = "prob")[, "PIGD"]
