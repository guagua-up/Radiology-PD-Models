...
### Data preparation
PIGD<- read.csv("D:/PD/Imagings/SUVR/CentrumSemiovale_1/HC_PIGD/HC_PIGD_Wholebrain/MODELS/FinalFeature_PIGD.csv")
PIGD <- PIGD[, -c(1:4)] 
PIGD[,1] <- factor(PIGD[,1], levels = c(1, 0), labels = c("Class1", "Class0"))
names(PIGD)[1] <- "Diagnosis"
str(PIGD)
PIGD |> count(Diagnosis)
miss_var_summary(PIGD)

set.seed(401)
resamples <- vfold_cv(data = PIGD,
                      v = 10,
                      repeats = 10,
                      strata = Diagnosis)
...
# Recipe
recipe_lr <- recipe(Diagnosis ~ ., data = PIGD) |>
...
# Recipe
recipe_svm <- recipe(Diagnosis ~ ., data = PIGD) |>
  step_scale(all_numeric_predictors()) |>  
  step_center(all_numeric_predictors())
...
# recipe
recipe_rf <- recipe(Diagnosis ~ ., data = PIGD) |>
  step_scale(all_numeric_predictors()) |>  
  step_center(all_numeric_predictors())
