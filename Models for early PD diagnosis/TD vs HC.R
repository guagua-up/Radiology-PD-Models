...
### Data preparation
TD <- read.csv("D:/PD/Imagings/SUVR/CentrumSemiovale_1/HC_TD/HC_TD_Wholebrain/MODELS/FinalFeature_TD.csv")
TD <- TD[, -c(1:5)] 
TD[,1] <- factor(TD[,1], levels = c(1, 0), labels = c("Class1", "Class0"))
...
# Recipe
recipe_lr <- recipe(Diagnosis ~ ., data = TD) |>
...
# Recipe
recipe_svm <- recipe(Diagnosis ~ ., data = TD) |>
  step_scale(all_numeric_predictors()) |>  
  step_center(all_numeric_predictors())
...
# recipe
recipe_rf <- recipe(Diagnosis ~ ., data = TD) |>
  step_scale(all_numeric_predictors()) |>  
  step_center(all_numeric_predictors())
